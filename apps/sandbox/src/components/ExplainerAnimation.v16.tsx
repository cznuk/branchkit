...
function OnScreenText({ text, visible }: { text: string; visible: boolean }) {
  ...
  return (
    <motion.div
      ...
      className="absolute left-8 top-1/2 -translate-y-1/2 z-50 pointer-events-none"
    >
      <div className="text-foreground text-5xl font-medium tracking-tight">
        {text}
      </div>
    </motion.div>
  );
...
const ANIMATION_TIMELINE: TimelineAction[] = [
  ...
  // 4. And again - Another fork, v4 appears, another visible change
  // ─────────────────────────────────────────────────────────────────────────────
  { type: "log", message: "And again" },
  { type: "hideText" },
  { type: "showText", text: "And again" },
  { type: "moveTo", target: "[data-new-version-button]", duration: 600 },
  { type: "wait", duration: 200 },
  { type: "click" },
  { type: "wait", duration: 200 },
