...
const { classifyTargetKind, getTargetIdFromPath } = require("./cli-helpers");
...
class ComponentManager {
  constructor(versionsFilePath, options = {}) {
    ...
    this.targetId = getTargetIdFromPath(path.join(this.watchDir, this.componentName), process.cwd());
    this.kind = classifyTargetKind(path.join(this.watchDir, `.tsx`));
...
class VersionSync {
  constructor(watchPath, options = {}) {
    ...
    this.components = new Map(); // Map<targetId, ComponentManager>
    this.wsClients = new Set();
    this.server = null;
    this.wss = null;
    this.lazy = options.lazy || false;
    this.port = options.port ?? process.env.PORT ?? 3030;
    this.componentMutationQueues = new Map();
    ...
    if (this.components.size === 0) {
      ...
      console.log("Run 'npx branchkit init <component-path>' to create one.");
    } else {
  ...
  discoverComponents() {
    ...
    for (const versionsFile of versionsFiles) {
      ...
      this.components.set(manager.targetId, manager);
    }
  }
  ...
  getComponent(identifier) {
    ...
    const direct = this.components.get(identifier);
    if (direct) return direct;

    const byName = [];
    for (const manager of this.components.values()) {
      if (manager.componentName === identifier) {
        byName.push(manager);
      }
    }
  ...
  queueComponentMutation(manager, operationName, task) {
    const key = manager?.targetId || "__unknown__";
    const previous = this.componentMutationQueues.get(key) || Promise.resolve();
    ...
    const next = previous
      ...
      .catch((error) => {
        console.error(
          `[WebSocket] ${operationName} queue error (${manager?.componentName || key}): ${error.message}`,
        );
      })
      .finally(() => {
        if (this.componentMutationQueues.get(key) === next) {
          this.componentMutationQueues.delete(key);
        }
      });

    this.componentMutationQueues.set(key, next);
  }
  ...
  startServer() {
    ...
    app.post("/open-in-editor", (req, res) => {
      const { version, targetId, component, editor: editorPreference } = req.body;

      try {
        if (!(targetId || component)) {
          return res.status(400).json({
            error: "Missing targetId/component parameter",
          });
        }

        const manager = this.getComponent(targetId || component);
        if (!manager) {
          return res.status(404).json({
            error: `Component not found: ${targetId || component}`,
          });
        }
    ...
    this.wss.on("connection", (ws) => {
      ...
      ws.send(
        JSON.stringify({
          type: "ack",
          payload: { message: "Connected to branchkit watch server" },
        }),
      );
  ...
  handleWebSocketMessage(ws, data) {
    ...
    const componentName = payload?.targetId || payload?.component;
    ...
    if (["duplicate_version", "delete_version", "new_version", "rename_version", "rename_label"].includes(type)) {
      if (!componentName) {
        ws.send(
          JSON.stringify({
            type: "error",
            payload: { message: "Missing targetId/component parameter" },
          }),
        );
        return;
      }
  ...
  handleDuplicateVersion(ws, payload) {
    const { version, newVersion, targetId, component } = payload;
    const manager = this.getComponent(targetId || component);
    const timestamp = new Date().toISOString();
    if (!manager) return;

    this.queueComponentMutation(manager, "duplicate_version", () => {
      try {
      if (!version) {
      ...
      if (newVersion && !manager.validateVersionKey(newVersion)) {
        throw new Error(`Invalid target version format: ${newVersion}`);
      }

      const sourceContent = fs.readFileSync(sourceFilePath, "utf8");
      const extension = path.extname(sourceFilePath);
      const { version: targetVersion, filePath: finalTargetPath } = this.writeVersionFileWithRetry(
        manager,
        {
          requestedVersion: newVersion,
          extension,
          buildContent: () => sourceContent,
          conflictErrorPrefix: "Target version already exists",
        },
      );
      ...
      manager.generateVersionsFile();
      this.broadcastFileChange(manager.targetId);
  ...
  handleDeleteVersion(ws, payload) {
    const { version, targetId, component } = payload;
    const manager = this.getComponent(targetId || component);
    const timestamp = new Date().toISOString();
    if (!manager) return;

    this.queueComponentMutation(manager, "delete_version", () => {
      try {
      if (!version) {
        throw new Error("Missing version parameter");
      }
  ...
  handleNewVersion(ws, payload) {
    ...
    this.queueComponentMutation(manager, "new_version", () => {
      try {
      if (version && !manager.validateVersionKey(version)) {
        throw new Error(`Invalid version format: ${version}`);
      }

      const extension = manager.getMostCommonExtension();
      const { version: targetVersion, filePath: finalFilePath } = this.writeVersionFileWithRetry(
        manager,
        {
          requestedVersion: version,
          extension,
          buildContent: (targetVersion) => {
...
`;
          ...
          conflictErrorPrefix: "Version already exists",
        },
      );
  ...
  writeVersionFileWithRetry(
    manager,
    { requestedVersion, extension, buildContent, conflictErrorPrefix = "Version already exists" },
  ) {
    const maxAttempts = requestedVersion ? 1 : 50;
    let lastConflictVersion = requestedVersion || null;

    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      const targetVersion =
        requestedVersion || manager.versionNumberToKey(manager.getNextVersionNumber());
      lastConflictVersion = targetVersion;
      ...
      try {
        fs.writeFileSync(finalFilePath, buildContent(targetVersion), { encoding: "utf8", flag: "wx" });
        return { version: targetVersion, filePath: finalFilePath };
      } catch (error) {
        if (error && error.code === "EEXIST") {
          if (requestedVersion) {
            throw new Error(`${conflictErrorPrefix}: ${targetVersion}`);
          }
          continue;
        }
        throw error;
    ...
    throw new Error(
      `Failed to allocate a new version after ${maxAttempts} attempts${
        lastConflictVersion ? ` (last conflict: ${lastConflictVersion})` : ""
      }`,
    );
  }
  ...
  handleRenameVersion(ws, payload) {
    ...
    this.queueComponentMutation(manager, "rename_version", () => {
      try {
  ...
  handleRenameLabel(ws, payload) {
    const { version, newLabel, targetId, component } = payload;
    const manager = this.getComponent(targetId || component);
    const timestamp = new Date().toISOString();
    if (!manager) return;

    this.queueComponentMutation(manager, "rename_label", () => {
      try {
      if (!version) {
        throw new Error("Missing version parameter");
      }
      ...
      const escapedLabel = JSON.stringify(newLabel).slice(1, -1);
      const updatedContent = content.replace(versionPattern, (_match, prefix, _current, suffix) => {
        return `${prefix}${escapedLabel}${suffix}`;
      });
  ...
  handlePromoteVersion(ws, payload) {
    ...
    this.queueComponentMutation(manager, "promote_version", () => {
      try {
      if (!version) {
        throw new Error("Missing version parameter");
      }
      ...
      this.components.delete(manager.targetId);
  ...
  handleFileChange(eventType, filePath) {
    ...
    if (eventType === "add" && filename.endsWith(".versions.ts")) {
      ...
      if (!this.components.has(manager.targetId)) {
        this.components.set(manager.targetId, manager);
        manager.generateVersionsFile();
        manager.updatePreviousVersionsData();
    ...
    if (eventType === "unlink" && filename.endsWith(".versions.ts")) {
      for (const [name, manager] of this.components) {
        if (manager.versionsFile === filePath) {
          ...
          console.log(`[Watch] Component removed: ${manager.targetId}`);
          this.broadcastFileChange(manager.targetId);
          break;
        }
      }
    ...
    for (const manager of this.components.values()) {
      ...
      if (isVersionFile) {
        console.log(`[${manager.targetId}] Detected change: ${eventType} ${filename}`);
