...
export function Tooltip({ label, children, placement = "top" }: TooltipProps) {
  ...
  // Find branchkit-root element for portal
  useEffect(() => {
    const rootEl = document.getElementById("branchkit-root");
    if (rootEl) {
  ...
  const portalRoot =
    portalRootRef.current || document.getElementById("branchkit-root") || document.body;
