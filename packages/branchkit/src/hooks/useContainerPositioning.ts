...
function calculateElementPosition(
  element: HTMLElement,
  branchkitRoot: HTMLElement | null,
  ...
  // Skip elements inside #branchkit-root
  if (branchkitRoot && branchkitRoot.contains(element)) {
    return null;
  }
...
function findFixedPositionElements(): DevToolPosition[] {
  ...
  const branchkitRoot = document.getElementById("branchkit-root");
